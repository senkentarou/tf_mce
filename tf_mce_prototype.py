...
if __name__ == "__main__":
    ...
    kmeans = KMeans(n_clusters=proto_num, n_init=20, random_state=0)
    proto = np.array([kmeans.fit(corrected_x[np.where(y==cls)]).cluster_centers_ for cls in range(class_num)])
    ...
    # Evaluation function g = (x - A)^2 = x^2 -2xA +A^2 => A^2 -2xA (x^2 is common to each classes) -> n * class * proto
    g = tf.reduce_sum(tf.map_fn(lambda x: tf.subtract(tf.multiply(A, x), tf.multiply(0.5, tf.square(A))), x_data), axis=3)

    # Select minimum g by proto direction -> n * class
    nearest_proto = tf.reduce_max(g, axis=2)
    nearest_proto_ind = tf.one_hot(tf.argmax(g, axis=2), proto_num)  # save indicator of nearest prototype by class

    # Extract g value of class i
    class_i_ind = y_target  # n * class
    g_i_value = tf.multiply(class_i_ind, nearest_proto)  # n * class
    g_not_i_value = tf.multiply(tf.ones_like(class_i_ind), -np.inf)  # n * class
    g_i = tf.where(tf.equal(g_i_value, 0), g_not_i_value, g_i_value)  # fill np.inf if g_i_value == 0 (= it is not class i values) -> n * class
    g_i_max = tf.reduce_max(g_i, axis=1, keepdims=True)  # n * 1

    # Extract g value of class j (j != i)
    class_j_ind = tf.subtract(tf.ones_like(y_target), y_target)  # n * class
    g_j_value = tf.multiply(class_j_ind, nearest_proto)  # n * class
    g_not_j_value = tf.multiply(tf.ones_like(class_j_ind), -np.inf)  # n * class
    g_j = tf.where(tf.equal(g_j_value, 0), g_not_j_value, g_j_value)  # fill np.inf if g_j_value == 0 (= it is not class j values) -> n * class
    g_j_max = tf.reduce_max(g_j, axis=1, keepdims=True)  # n * 1
    class_j_max_ind = tf.one_hot(tf.argmax(g_j, axis=1), class_num)  # save indicator of g_j_value
    ...
    diff_l = tf.multiply(alpha, tf.multiply(l, tf.subtract(1.0, l)))  # l' = alpha * l * (1.0 - l) -> n * 1
    diff_d_proto_sign = tf.subtract(class_j_max_ind, class_i_ind)  # n * class
    delta_d_proto = tf.multiply(diff_d_proto_sign, diff_l) # n * class
    diff_d_proto_value = tf.expand_dims(delta_d_proto, axis=2)  # n * class * 1
    diff_d_value = tf.expand_dims(tf.multiply(nearest_proto_ind, diff_d_proto_value), axis=3)  # n * class * proto * 1

    sub_x_p = tf.map_fn(lambda x: tf.add(tf.multiply(-1.0, A), x), x_data)  # n * class * proto * d
    delta_x_p = tf.multiply(sub_x_p, diff_d_value)  # n * class * proto * d
    diff_d = tf.reduce_mean(delta_x_p, axis=0)  # class * proto * d
    diff_A = tf.multiply(rho, diff_d)  # class * proto * d
    A_next = tf.assign(A, tf.subtract(A, diff_A))  # Updating next A value -> class * proto * d
    ...
    print ("  * Epoch: %s" % epoch)
    print ("  * Prototype size: %s" % proto_num)
    ...
    for n in range(epoch+1):
        ...
        if n % observation_lapse == 0:
            ...
            ax0.scatter(A_values[0, :, 0], A_values[0, :, 1], marker="s", s=50, c="blue", label="class blue proto")
            ax0.scatter(A_values[1, :, 0], A_values[1, :, 1], marker="s", s=50, c="green", label="class green proto")
